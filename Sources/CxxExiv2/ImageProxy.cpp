...
static const String kGPSImgDirectionKey     = "Exif.GPSInfo.GPSImgDirection";
static const String kGPSImgDirectionRefKey  = "Exif.GPSInfo.GPSImgDirectionRef";
...
optional<DateTime> ImageProxy::getTimeStamp(const String& dateTimeKeyName, const String& offsetKeyName) const {
...
optional<Direction> ImageProxy::getDirection() const {
    auto val = getValueForExifKey(kGPSImgDirectionKey);
    auto ref = getValueForExifKey(kGPSImgDirectionRefKey);

    if (!val || !ref) return nullopt;

    Direction result;
    result.type = ref->toString() == "T" ? Direction::Type::kTrue : Direction::Type::kMagnetic;
}

void ImageProxy::setDirection(optional<Direction> opt_dir) {
    if (!opt_dir) {
        setValueForExifKey(kGPSImgDirectionKey, nullptr);
        setValueForExifKey(kGPSImgDirectionRefKey, nullptr);
    }
    else {
        Direction dir = opt_dir.value();
        auto directionVal = makeValueFromRational(dir.value);
        auto directionRef = makeValueFromString(dir.type == Direction::Type::kTrue ? "T" : "M");

        setValueForExifKey(kGPSImgDirectionKey, directionVal.release());
        setValueForExifKey(kGPSImgDirectionRefKey, directionRef.release());
    }
}
